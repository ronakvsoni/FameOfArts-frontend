document.addEventListener("DOMContentLoaded", () => {
  ...
  function navButtons() {
    ...
    document
      .getElementById("uplaod-image-button")
      .addEventListener("click", function () {
    ...
    document
      .getElementById("add-artgallary-button")
      .addEventListener("click", function () {
        document.body.scrollTop = document.documentElement.scrollTop = 0;
  ...
  function buildAccountPage() {
    ...
    loggedInUser.artgallaries.forEach((fridge) => {
    ...
    function addFridgeToCollection(fridge) {
      ...
      div2.className = "image";
      img.src =
        "https://previews.123rf.com/images/rastudio/rastudio1803/rastudio180300624/97357825-easel-vector-line-icon-isolated-on-white-background-artistic-equipment-empty-artist-canvas-on-easel-.jpg";
  ...
  function buildFridgePage(id) {
    fetch(`http://localhost:3000/artgallaries/${id}`)
      .then((resp) => resp.json())
      .then((json) => {
    ...
    function populateFridge(fridge) {
      ...
      let fridgeDiv = document.getElementById("artgallary-display");
      ...
      function showImg(image) {
        ...
        document.getElementById("description").innerText =
          image.data.attributes.description;
  ...
  function buildUploadImgPage() {
    ...
    form.addEventListener("submit", function (event) {
      ...
      if (imageFile && event.target.fridge_selection.value != "null") {
        ...
        alert(
          "Please make sure you have Uploaded an Image and Chosen a fridge"
        );
      }
    });
  ...
  function buildNewFridgePage() {
    replaceable.innerHTML = newArtgallaryPage();
    ...
    document
      .getElementById("new_fridge_form")
    ...
    function buildFridge(e) {
      ...
      const data = {
        url:
          "https://www.graphicsprings.com/filestorage/stencils/aeaa0c83a48a435bb729308d72d41ee7.svg",
      ...
      fetch("http://localhost:3000/artgallaries", configObj)
        .then((resp) => resp.json())
        .then(refreshUser)
        .catch((error) => {
          alert(error);
